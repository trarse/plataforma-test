...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (authToken) {
        // Si el usuario ya está logueado, carga todo
...
async function loadInitialData() {
    try {
        ...
        // Solo cuando los datos están listos, se inicializa la app principal
        initializeApp();
        ...
        console.error("Fallo crítico en la carga inicial, cerrando sesión.", error);
        logout();
    }
}

// Esta función SOLO se llama para un usuario ya logueado
...
// Esta función SOLO se llama para un usuario que necesita loguearse
function setupAuthForm() { 
    ...
    const updateForm = () => { 
        ...
        toggleText.innerHTML = isLogin ? '¿No tienes cuenta? <a href="#" id="auth-toggle-link">Regístrate</a>' : '¿Ya tienes cuenta? <a href="#" id="auth-toggle-link">Inicia Sesión</a>'; 
...
function displayExamReview(examData) { showSection('exam-review'); document.getElementById('review-title').textContent = `Revisión del Simulacro (${examData.date})`; const reviewArea = document.getElementById('review-area'); reviewArea.innerHTML = ''; const questionsMap = new Map(ALL_TEST_QUESTIONS.map(q => [q._id, q])); const questionsInExam = examData.questionIds.map(id => questionsMap.get(id)).filter(Boolean); questionsInExam.forEach((q, index) => { const userAnswerIndex = examData.userAnswers[q._id]; const userAnswerText = userAnswerIndex !== undefined ? q.opts[userAnswerIndex] : "Sin responder"; const isCorrect = userAnswerText === q.c; const questionDiv = document.createElement('div'); questionDiv.className = 'test-question'; let optionsHTML = q.opts.map((opt, optIdx) => { let className = ''; if (opt === q.c) className = 'correct'; else if (optIdx === userAnswerIndex) className = 'incorrect'; return `<button class="${className}" disabled>${opt}</button>`; }).join(''); questionDiv.innerHTML = `<h3>Pregunta ${index + 1}</h3> <p>${q.q}</p><div class="test-options">${optionsHTML}</div><div class="explanation" style="display: block;"><strong>Tu respuesta:</strong> <span class="${isCorrect ? 'stat-correct' : 'stat-incorrect'}">${userAnswerText}</span><br><strong>Explicación:</strong> ${q.expl || 'No disponible.'}</div>`; reviewArea.appendChild(questionDiv); }); }
...
async function deleteQuestion(questionId) { if (!confirm('¿Estás SEGURO de que quieres borrar esta pregunta de forma PERMANENTE?')) return; const result = await apiRequest(`questions/${questionId}`, 'DELETE'); if (result) { alert('Pregunta borrada con éxito.'); ALL_TEST_QUESTIONS = ALL_TEST_QUESTIONS.filter(q => q._id !== questionId); const indexCurrent = currentQuestions.findIndex(q => q._id === questionId); if (indexCurrent > -1) { currentQuestions.splice(indexCurrent, 1); const questionDiv = document.getElementById(`question-${indexCurrent}`); if (questionDiv) questionDiv.remove(); } } else { alert('Error al borrar la pregunta.'); } }
...
function importProgress() { const fileInput = document.getElementById('import-progress-input'); const file = fileInput.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = async (event) => { try { const data = JSON.parse(event.target.result); if (confirm('¿Seguro que quieres importar este progreso? Se sobrescribirán tus datos actuales.')) { userStats = data.userStats || {}; markedQuestions = data.markedQuestions || []; incorrectQuestions = data.incorrectQuestions || []; examHistory = data.examHistory || []; savedUserStats = data.savedUserStats || null; lastDailyChallenge = data.lastDailyChallenge || ''; await saveProgressToServer(); alert('Progreso importado con éxito. La página se recargará.'); location.reload(); } } catch (e) { alert('Error: El archivo no es un JSON válido.'); } }; reader.readAsText(file); }
